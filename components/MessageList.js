  ...
  render() {        
    ...
    return (
      <div>
        {messages.map(m => (
          <div key={m.id}>
            <div style={{ marginLeft: m.createdByUserId === currentUserId ? '40%' : '0', marginRight: m.createdByUserId === currentUserId ? '0' : '40%', marginBottom: '15px' }}>
              <p style={{ color: m.createdByUserId === authorId ? 'purple' : 'black', marginBottom: '3px' }}>{m.text}</p>
